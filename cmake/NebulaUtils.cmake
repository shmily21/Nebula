...
# Usage samples:
#   nebula_assert(MyLib_FOUND)
#   nebula_assert(DEFINED MyLib_INCLUDE_DIRS)
macro(nebula_assert)
    if(NOT (${ARGN}))
...
macro(nebula_check_environment_variables)
    foreach(_var ${ARGN})
...
# adds include directories in such way that directories from the Nebula source tree go first
function(nebula_include_directories)
    set(__add_before "")
    foreach(dir ${ARGN})
        get_filename_component(__abs_dir "${dir}" ABSOLUTE)
        if("${__abs_dir}" MATCHES "^${Nebula_SOURCE_DIR}" OR "${__abs_dir}" MATCHES "^${Nebula_BINARY_DIR}")
...
macro(nebula_clear_vars)
    foreach(_var ${ARGN})
        unset(${_var} CACHE)
    endforeach()
endmacro()

set(NEBULA_COMPILER_FAIL_REGEX
...
MACRO(nebula_check_compiler_flag LANG FLAG RESULT)
    if(NOT DEFINED ${RESULT})
        ...
        if(_fname)
            ...
            FOREACH(_regex ${NEBULA_COMPILER_FAIL_REGEX})
...
macro(nebula_check_flag_support lang flag varname)
    if("_${lang}_" MATCHES "_CXX_")
        set(_lang CXX)
    elseif("_${lang}_" MATCHES "_C_")
        set(_lang C)
    ...
    nebula_check_compiler_flag("${_lang}" "${ARGN} ${flag}" ${${varname}})
endmacro()

# turns off warnings
macro(nebula_warnings_disable)
        ...
        elseif((CMAKE_COMPILER_IS_GNUCXX OR (UNIX AND CV_ICC)) AND _gxx_warnings AND _flag_vars)
            foreach(var ${_flag_vars})
                foreach(warning ${_gxx_warnings})
                    ...
                    nebula_check_flag_support(${var} "${warning}" _varname)
                    if(${_varname})
                        set(${var} "${${var}} ${warning}")
                    endif()
                endforeach()
...
macro(NEBULA_OPTION variable description value)
    set(__value ${value})
    set(__condition "")
...
set(NEBULA_BUILD_INFO_FILE "${Nebula_BINARY_DIR}/version_string.tmp")
file(REMOVE "${Nebula_BUILD_INFO_FILE}")
function(nebula_output_status msg)
    message(STATUS "${msg}")
    string(REPLACE "\\" "\\\\" msg "${msg}")
    string(REPLACE "\"" "\\\"" msg "${msg}")
    file(APPEND "${NEBULA_BUILD_INFO_FILE}" "\"${msg}\\n\"\n")
endfunction()

macro(nebula_finalize_status)
    if(NOT NEBULA_SKIP_STATUS_FINALIZATION)
        if(TARGET NebulaCore)
            execute_process(COMMAND ${CMAKE_COMMAND} -E copy_if_different "${NEBULA_BUILD_INFO_FILE}" "${NebulaCore_BINARY_DIR}/version_string.inc" OUTPUT_QUIET)
        endif()
    endif()
endmacro()
    ...
    if(DEFINED status_cond)
        ...
        if(DEFINED status_then OR DEFINED status_else)
            if(${status_cond})
                ...
                nebula_output_status("${status_text} ${status_then}")
            else()
                string(REPLACE ";" " " status_else "${status_else}")
                string(REGEX REPLACE "^[ \t]+" "" status_else "${status_else}")
                nebula_output_status("${status_text} ${status_else}")
            endif()
        else()
            string(REPLACE ";" " " status_cond "${status_cond}")
            string(REGEX REPLACE "^[ \t]+" "" status_cond "${status_cond}")
            nebula_output_status("${status_text} ${status_cond}")
        endif()
    else()
        nebula_output_status("${text}")
    endif()
...
macro(nebula_split_libs_list lst lstdbg lstopt)
    set(${lstdbg} "")
    set(${lstopt} "")
    set(perv_keyword "")
    foreach(word ${${lst}})
...
macro(nebula_list_filterout lst regex)
    foreach(item ${${lst}})
...
macro(nebula_list_unique __lst)
    if(${__lst})
...
macro(nebula_list_reverse __lst)
    if(${__lst})
        list(REVERSE ${__lst})
    endif()
endmacro()
...
macro(nebula_list_add_prefix LST PREFIX)
    set(__tmp "")
    foreach(item ${${LST}})
...
macro(nebula_list_add_suffix LST SUFFIX)
    set(__tmp "")
    foreach(item ${${LST}})
...
macro(nebula_list_pop_front LST VAR)
    if(${LST})
        list(GET ${LST} 0 ${VAR})
...
macro(nebula_regex_escape var regex)
    string(REGEX REPLACE "([+.*^$])" "\\\\1" ${var} "${regex}")
endmacro()


# convert list of paths to full paths
macro(nebula_convert_to_full_paths VAR)
    if(${VAR})
        set(__tmp "")
        foreach(path ${${VAR}})
...
# add install command
function(nebula_install_target)
...
macro(nebula_parse_header FILENAME FILE_VAR)
    set(vars_regex "")
...
macro(nebula_parse_header2 LIBNAME HDR_PATH VARNAME)
    nebula_clear_vars(${LIBNAME}_VERSION_MAJOR
...
macro(nebula_parse_pkg LIBNAME PKG_PATH SCOPE)
...
function(nebula_source_group group)
    cmake_parse_arguments(NEBULA_SOURCE_GROUP "" "" "GLOB" ${ARGN})
    file(GLOB srcs ${NEBULA_SOURCE_GROUP_GLOB})
